...
class Relatorios extends CI_Controller
{
    ...
    public function clientes()
    {
        ...
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'rCliente')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de associados.');
            redirect(base_url());
        }
    ...
    public function clientesCustom()
    {
        ...
        $data['title'] = 'Relatório de Associados Costumizado';
        $data['dataInicial'] = date('Y/m/d', strtotime($dataInicial));
        $data['dataFinal'] = date('Y/m/d', strtotime($dataFinal));
    ...
    public function clientesRapid()
    {
        ...
        $data['title'] = 'Relatório de Associados';
        ...
        $html = $this->load->view('relatorios/imprimir/imprimirClientes', $data, true);
        pdf_create($html, 'relatorio_clientes' . date('Y/m/d'), true);
    }
    ...
    public function osCustom()
    {
        ...
        $associacao = $this->input->get('associacao');

        $this->load->helper('mpdf');
        
        $title = $associacao == null ? 'Todas' : $associacao;
        $user =  $responsavel == null ? 'Não foi selecionado' : $this->Usuarios_model->get(1, intval($responsavel) - 1);
        
        $os = $this->Relatorios_model->osCustom($dataInicial, $dataFinal, $cliente, $responsavel, $associacao);
