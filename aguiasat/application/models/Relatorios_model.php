...
class Relatorios_model extends CI_Model
{
    ...
    public function osRapid()
    {
        ...
        $this->db->select('os.*,clientes.nomeCliente, usuarios.nome,
            total_servicos.total_servico, total_produtos.total_produto');
        $this->db->from('os');
    ...
    public function osCustom($dataInicial = null, $dataFinal = null, $cliente = null, $responsavel = null, $associacao = null)
    {
        $whereData = "";
        $whereCliente = "";
        $whereResponsavel = "";
        ...
        if ($associacao != null) {
            $whereAssociacao = "AND associacao = " . $this->db->escape($associacao);
        }
        $query = 'CREATE TEMPORARY TABLE IF NOT EXISTS total_produtos SELECT SUM(subTotal) as total_produto, os_id FROM produtos_os GROUP BY os_id; ';
        $this->db->query($query);
        ...
        $query = "SELECT os.*,clientes.nomeCliente, usuarios.nome ,total_servicos.total_servico, total_produtos.total_produto FROM os
                   ...
                   LEFT JOIN usuarios ON os.usuarios_id = usuarios.idUsuarios
                   WHERE idOs != 0 $whereData $whereCliente $whereResponsavel $whereAssociacao";
